...
(async () => {
  ...
  try {
    ...
    // If gtag exists, trigger a manual debug page_view to force a collect request
    if (hasData.hasGtag) {
      console.log('Triggering manual page_view (debug_mode:true)');
      await page.evaluate(() => {
        try {
          window.gtag('event', 'page_view', { page_location: window.location.href, debug_mode: true });
        } catch (e) { console.error('gtag call failed', e); }
      });

      try {
        const req = await page.waitForRequest((r) => r.url().includes('/g/collect') || r.url().includes('/collect'), { timeout: 8000 });
        collectRequests.push({ url: req.url(), method: req.method(), headers: req.headers(), timestamp: new Date().toISOString() });
      } catch (e) {
